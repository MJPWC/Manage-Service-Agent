...
(function() {
  ...
  function renderServiceNowTicketPopup(eventId, incidentData, appName, extraWorkNotes = "") {
    console.log("[ServiceNow Popup] Rendering create popup:", {
      eventId,
      appName,
      incidentData,
    });
    ...
    const baseWorkNotes = incidentData.work_notes || incidentData.workNotes || "";
    const appendedWorkNotes = extraWorkNotes
      ? `${baseWorkNotes}\n\n---\n${extraWorkNotes}`.trim()
      : baseWorkNotes;
    ...
    const modalHtml = `
      <div class="service-now-ticket-modal" id="serviceNowTicketPopup">
        ...
        <div class="service-now-content">
          <div class="service-now-header">
            <h3>Create ServiceNow Ticket</h3>
            <button class="btn-icon" id="closeServiceNowPopup" title="Close">✕</button>
          </div>
          <div class="service-now-body">
            <div class="ticket-info">
            ...
            <div class="ticket-fields">
              <div class="field-group">
                <label for="shortDescription">Short Description:</label>
                <textarea id="shortDescription" class="ticket-textarea" rows="2" maxlength="80">${window.escapeHtml(incidentData.short_description || incidentData.shortDescription || "")}</textarea>
                <div class="char-count"><span id="shortDescCount">0</span>/80</div>
              </div>
              <div class="field-group">
                <label for="description">Description:</label>
          ...
          <div class="service-now-footer">
            <button class="btn btn-secondary" id="cancelServiceNowTicket">Cancel</button>
            <button class="btn btn-primary" id="createServiceNowTicket">Create Ticket</button>
          </div>
        </div>
      </div>
    `;
    ...
    document.body.insertAdjacentHTML("beforeend", modalHtml);
    setupEventListeners(eventId, incidentData, false);
  }

  function closeServiceNowTicketPopup() {
    const popup = document.getElementById("serviceNowTicketPopup");
    if (popup) {
      popup.remove();
    }
  }
  ...
  async function showServiceNowTicketPopup(
    ...
    try {
      ...
      alert(`Error preparing ticket: ${error.message}`);
    }
  }
  ...
  async function createServiceNowTicketFromPopup(eventId) {
    try {
      ...
      if (response.success) {
        ...
        // Refresh the incident action container when the event details view exposes it.
        if (typeof window.populateIncidentActionContainer === "function") {
          window.populateIncidentActionContainer(eventId);
        }
      } else {
        alert(
