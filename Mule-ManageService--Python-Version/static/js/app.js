...
(function () {
  ...
  function collectExpectedUploadFiles(logs) {
    ...
    (logs || []).forEach((log) => {
      const fileName = getErrorFileNameFromLog(log);
  ...
  window.setEventDetailsFlowState = setEventDetailsFlowState;
  ...
  function closeEventDetailsModal() {
    window.AppModalUtils.closeEventDetailsModal();

    if (state.loadingRequests > 0 && elements.loadingOverlay) {
      state.loadingRequests = 0;
      elements.loadingOverlay.classList.add("hidden");
    }

    if (state.currentTab === "mulesoft") {
      if (state.logs && state.logs.length > 0) {
        renderLogs();
      } else if (state.selectedAppId && state.currentEnvId) {
    ...
    } else if (state.currentTab === "correlation") {
      renderCorrelationIds();
    }
  ...
  window.openFileFromLocal = function () {
    ...
    const expectedFiles = window.__eventDetailsContext?.expectedUploadFiles || [];
