...
(function () {
      ...
      .map((app) => {
        ...
        <div class="api-item ${isActive ? "active" : ""}" data-id="${app.id}">
          ...
          <div class="api-header">
            ...
            <div class="api-name">${escapeHtml(app.name)}</div>
  ...
  function getStatusClass(status) {
    ...
    if (s === "RUNNING" || s === "STARTED" || s === "APPLIED") return "running";

    if (s === "STOPPED" || s === "UNDEPLOYED") return "stopped";
  ...
  function groupLogsByCorrelationId(logs) {
    ...
    logs.forEach((log, originalIndex) => {
      ...
      if (!grouped[correlationId]) {
        grouped[correlationId] = {
          ...
          firstLog: log, // Use first occurrence for primary info
    ...
    // Convert to array and sort by first occurrence (newest first)
  ...
  function renderLogs() {
    ...
    // Filter out logs with missing correlation ID (N/A) - must match the logic in calculateFilteredErrorCount

    filteredLogs = filteredLogs.filter((log) => hasValidEventId(log));

    // Filter to only show logs that have actual error details
    ...
    const selectedApp = state.applications.find(
      (a) => a.id === state.selectedAppId,
    );
    ...
    // Generate HTML for each correlation ID group with AI summary

    const logsHtml = groupedLogs

      .map((group, groupIndex) => {
        const log = group.firstLog; // Use first log in group for display

        const index = group.originalIndices[0]; // Use first original index
        ...
        const errorCount = group.errorCount; // Number of errors with this correlation ID
        ...
        // Try to extract API name and file name from FlowStack analysis first

        const flowStackInfo = extractApiAndFileFromFlowStack(eventId);

        if (flowStackInfo.apiName) {
          apiName = flowStackInfo.apiName;
        }

        if (flowStackInfo.fileName) {
          filename = flowStackInfo.fileName;
        }

        // Fallback to exception parsing if FlowStack analysis didn't provide the info

        if (exception) {
          // Try multiple field names for error type
          ...
          // If still no message, try from error field

          if (!errorMessage && exception.error) {
            errorMessage = exception.error;
          }

          // Only extract from Element if we didn't get filename from FlowStack
          ...
          // Extract API name from exception Element if we still don't have it
        ...
        // Try application field from log
        ...
        // NEVER use the file name (appName) as API name - only use if absolutely nothing else found
        ...
        console.log(
          `[RenderLogs] Group ${groupIndex} (CorrelationID: ${eventId}): errorType="${errorType}", hasException=${!!exception}, errorCount=${errorCount}, apiName="${apiName}", fileName="${filename}"`,
        );

        // Extract all unique error messages from logs in this group
        ...
        group.logs.forEach((l) => {
          ...
          if (l.exception && l.exception.Message) {
            msg = l.exception.Message;
          } else if (l.exception && l.exception.message) {
            msg = l.exception.message;
          } else if (l.exception && l.exception.error) {
            msg = l.exception.error;
          }

          if (msg && msg.length > 0 && !seenMessages.has(msg)) {
        ...
        // Generate HTML for descriptions - show all line by line

        const descriptionHtml = errorMessages

          .map((msg, idx) => {
            const fullMsg = escapeHtml(msg);

            return `<div style="margin: ${idx > 0 ? "6px 0 0 0" : "0"}; padding: ${idx > 0 ? "6px 0 0 0" : "0"}; ${idx > 0 ? "border-top: 1px solid #e5e7eb;" : ""}">${fullMsg}</div>`;
          })

          .join("");

        // Generate list of all timestamps for this correlation ID
        ...
        // Check if this correlation ID is marked as expected error
        ...
        <div class="error-card${expectedErrorClass}" data-correlation-id="${escapeHtml(eventId)}" data-group-index="${groupIndex}">
          ...
          <div class="error-card-content">
            ...
            <div class="error-card-row">
              ...
              <span class="error-card-value">${escapeHtml(apiName)}</span>
            ...
            <div class="error-card-row">
              ...
              <span class="error-card-value error-type-badge">${escapeHtml(errorType)}</span>
            ...
            <div class="error-card-row">
              ...
              <span class="error-card-value">${escapeHtml(filename)}</span>
            ...
            <div class="error-card-row">
              ...
              <code class="correlation-id-badge event-id-clickable" data-event-id="${escapeHtml(eventId)}" title="Click to view details">${escapeHtml(eventId || "N/A")}</code>
    ...
    // Add toggle handlers for occurrences
  ...
  // Correlation IDs rendering

  function renderCorrelationIds() {
    // Remove environment requirement since correlation IDs are global
    ...
    const sorted = [...state.correlationData].sort((a, b) => {
      ...
      const ta = a.latest_timestamp || "";

      const tb = b.latest_timestamp || "";

      return tb.localeCompare(ta);
    });

    // Get statuses from localStorage (use default env if none selected)
    ...
    const rowsHtml = sorted
      .map((c, index) => {
        const apps = (c.applications || []).join(", ");

        const ts = formatTimestamp(c.latest_timestamp || "");

        const lastMsg = (c.last_message || "").substring(0, 160);

        const currentStatus = statuses[c.event_id] || "pending";

        const statusClass = getStatusClassForStatus(currentStatus);

        // RCA cell — truncate to 160 chars with full text in tooltip
        const rcaText = (c.rca || "").trim();
        const rcaTruncated =
        ...
        // Assignment group badge — highlight Muledev in accent colour
        const agName = c.assignmentGroup || "—";
        const isMuledev = agName.toLowerCase() === "muledev";
        const agHtml = `<span class="snow-ag-badge${isMuledev ? " snow-ag-muledev" : ""}">${escapeHtml(agName)}</span>`;

        const ticketHtml = c.incidentNumber
          ? `<a class="snow-ticket-link" href="${state.servicenowBaseUrl}/incident.do?sys_id=${c.incidentSysId || ""}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" title="Open ${c.incidentNumber} in ServiceNow">${c.incidentNumber}</a> <span class="snow-status-badge snow-status-${getSnowStatusClass(c.incidentStatus)}">${getSnowStatusText(c.incidentStatus)}</span>`
          : `<span class="snow-no-ticket">—</span>`;

        return `

        <div class="correlation-row" data-event-id="${escapeHtml(c.event_id)}" data-has-correlation-id="${c.has_correlation_id ? "true" : "false"}">

          <div class="correlation-cell index">#${index + 1}</div>

          <div class="correlation-cell id">

            <code class="correlation-id-badge event-id-clickable" data-event-id="${escapeHtml(c.event_id)}" title="${c.has_correlation_id ? `Click to view full chain - Full ID: ${escapeHtml(c.event_id)}` : `Incident number fallback (no correlation_id): ${escapeHtml(c.event_id)}`}">
          ...
          <div class="correlation-cell count">${c.count}</div>

          <div class="correlation-cell apps">${escapeHtml(apps || "—")}</div>

          <div class="correlation-cell ag">${agHtml}</div>

          <div class="correlation-cell time">${escapeHtml(ts || "—")}</div>

          <div class="correlation-cell status">

            <select class="status-select" data-event-id="${escapeHtml(c.event_id)}" value="${escapeHtml(currentStatus)}" onclick="event.stopPropagation()">
          ...
          <div class="correlation-cell message" title="${escapeHtml(c.last_message || "")}">
    ...
    // Row click => open event details modal (reusing existing handler)
    ...
    elements.correlationContent
      ...
      .forEach((row) => {
        row.addEventListener("click", async (e) => {
          ...
          const hasCorrelationId = target.dataset.hasCorrelationId === "true";
    ...
    elements.correlationContent
      ...
      .forEach((select) => {
        select.addEventListener("change", (e) => {
          ...
          updateCorrelationStatus(envId, eventId, newStatus);
  ...
  function getCorrelationStatuses(envId) {
    ...
    try {
      ...
      console.error("Error loading correlation statuses:", e);
  ...
  function updateCorrelationStatus(envId, eventId, status) {
    ...
    try {
      ...
      syncStatusToBackend(envId, eventId, status);
    } catch (e) {
      console.error("Error updating correlation status:", e);
    }
  }
  ...
  function handleExpectedErrorToggle(eventId) {
    ...
    if (currentStatus === "expected") {
      // Already marked as expected, ask if user wants to unmark
      ...
      if (shouldUnmark) {
        ...
        syncStatusToBackend(envId, eventId, "");
        ...
        // User said no, just close the panel
        closeEventDetailsModal();
      }
    } else {
      // Mark as expected error
  ...
  function updateErrorCardStyling(eventId, isExpected) {
    // Find and update error cards with this correlation ID
    const errorCards = document.querySelectorAll(`[data-correlation-id="${eventId}"]`);
  ...
  function refreshCurrentView() {
    // Refresh the appropriate view based on current tab
    const currentTab = state.currentTab;
    
    if (currentTab === "mulesoft") {
      renderLogs();
  ...
  async function generateErrorSummary(log, index, appName, options = {}) {
    ...
    try {
      ...
      if (window.__eventDetailsContext) {
        window.__eventDetailsContext.summaryRequestInFlight = true;
      }
      ...
      if (window.__eventDetailsContext) {
        window.__eventDetailsContext.summaryRequestInFlight = false;
      }
  ...
  async function loadCorrelationIds() {
    try {
      ...
      if (result && result.success) {
        ...
        // Store the ServiceNow base URL for linking to incidents
        ...
        state.correlationData = (result.correlationIds || []).map((item) => ({
          ...
          count: 1, // Default count, could be enhanced later
      ...
      console.error("Error loading correlation IDs:", err);
  ...
  function verifyAndFixButtonStates() {
    console.log("Verifying and fixing button states...");

    // Check all critical interactive elements

    const criticalElements = [
      { id: "envSelect", requiredAuth: false },

      { id: "refreshBtn", requiredAuth: true },

      { id: "logoutBtn", requiredAuth: true },

      { id: "apiSearch", requiredAuth: true },
    ];

    criticalElements.forEach(({ id, requiredAuth }) => {
      const element = document.getElementById(id);

      if (element) {
        const shouldBeEnabled = !requiredAuth || state.authenticated;
    ...
    tabButtons.forEach((btn, index) => {
      ...
      const shouldBeEnabled = tabName === "mulesoft" || state.authenticated;
  ...
  async function loadApplications(envId) {
    ...
    if (envId === "local") {
      return await loadLocalApplications();
    }
    ...
    try {
      const result = await api(
        ...
        `/api/environments/${envId}/applications`,
      );
      ...
      if (result.success) {
        ...
        // Fetch error counts in background
  ...
  async function loadLocalApplications() {
    ...
    try {
      // Reset time filter for local environment
      ...
      if (result.success) {
        ...
        // Get error counts for local application
        ...
        if (errorResult.success) {
          state.errorCounts = errorResult.errorCounts || {};
        } else {
          state.errorCounts = {};
        }
        ...
        // Load logs for the first application and auto-select it

        if (state.applications.length > 0) {
          const appId = state.applications[0].id;

          const logsResult = await api(
            "GET",
  ...
  async function fetchErrorCounts(envId) {
    try {
      const result = await api(
        ...
        `/api/environments/${envId}/error-counts`,
      );

      if (result.success && result.errorCounts) {
        state.errorCounts = result.errorCounts;

        // Fetch actual logs for all apps so error counts are calculated correctly

        // This populates state.appLogs with the actual log entries for filtering
        ...
        for (const app of state.applications) {
          try {
            ...
            console.error(`Failed to fetch logs for ${app.id}:`, err);

            // Keep the app but mark logs as empty

            state.appLogs[app.id] = [];
          }
        }

        // Re-render with correct error counts now that appLogs is populated

        renderApplications();
      }
    } catch (err) {
      console.error("Failed to fetch error counts:", err);
    }
  }
  ...
  async function selectApplication(appId) {
    ...
    // Load logs first to populate state.appLogs[appId] before rendering

    await loadLogs(appId);

    // Then render applications with correct error counts

    renderApplications();

    // Start auto-refresh when an application is selected

    startAutoRefresh();
  }

  async function loadLogs(appId) {
    if (!appId || !state.currentEnvId) return;
    ...
    try {
      // Build API endpoint with optional time range parameters
      ...
      if (result.success) {
        ...
        // Store logs for this app to be used in filtered count calculations

        state.appLogs[appId] = logs;

        state.rawLogText = result.rawText || ""; // Store raw log text from API response

        state.logAnalysis = result.analysis || {}; // Store analysis data from API response

        console.log(
          "[LoadLogs] Raw log text available:",

          state.rawLogText.length > 0,
        );

        console.log("[LoadLogs] Analysis data available:", !!state.logAnalysis);
  ...
  function startAutoRefresh() {
    ...
    state.autoRefreshInterval = setInterval(async () => {
      if (state.authenticated && state.currentEnvId) {
        console.log("Auto-refreshing all applications...");
  ...
  function updateLastRefreshDisplay() {
    ...
    if (elements.lastRefreshDisplay) {
      elements.lastRefreshDisplay.textContent = displayText;
    }
  }
  ...
  async function loadGitHubRepos() {
    ...
    // Check if repos are already loaded
  ...
  async function loadRepositoryContents(owner, repoName, path = "") {
    // First refresh session to get latest authentication state
  ...
  // Function to run analysis using error-analysis-rules.txt (for AI Analysis Modal)

  async function runAnalysisWithRules(file, originalErrorText) {
    const resultDiv = document.getElementById("errorAnalysisResult");

    if (!resultDiv) return;

    resultDiv.innerHTML =
      '<div class="loading">Generating enhanced analysis...</div>';

    try {
      const payload = {
        content: file.content, // Use the AI analysis response as content

        file_path: file.path,

        reference_file_content: originalErrorText, // Use original error as reference
      ...
      if (result.success) {
        ...
        const currentContent =
          resultDiv.querySelector(".analysis-text")?.textContent || "";

        // Append the new analysis if it contains code fixes

        if (
          formattedAnalysis.includes("```") ||
          formattedAnalysis.toLowerCase().includes("fix") ||
          formattedAnalysis.toLowerCase().includes("code")
        ) {
          // Add separator and append new analysis

          resultDiv.innerHTML = `

            <div class="analysis-text">${currentContent}</div>

            <div style="margin-top: 20px; padding-top: 20px; border-top: 2px solid var(--border-color);">

              <h4 style="color: var(--accent-blue); margin-bottom: 12px;">🔧 Code Fixes & Enhanced Analysis</h4>
          ...
          // No code fixes => just append a "No additional fixes" message

          resultDiv.innerHTML = `

            <div class="analysis-text">${currentContent}</div>

            <div style="margin-top: 16px; padding: 12px; background: var(--bg-tertiary); border-radius: 6px; text-align: center; color: var(--text-secondary);">

              No additional code fixes found in enhanced analysis.
  ...
  async function doApplyChanges(
    ...
    try {
      ...
      if (response.success) {
        ...
        // Set ticketContextForPR for ServiceNow integration (GitHub analysis flow)
        // Use the actual error context if available from the analysis
        if (window.__eventDetailsContext && window.__eventDetailsContext.logs) {
          state.ticketContextForPR = {
            eventId: window.__eventDetailsContext.eventId || "github-analysis-" + Date.now(),
            logs: window.__eventDetailsContext.logs,
            appName: window.__eventDetailsContext.appName || file.repo || "GitHub Analysis"
          };
        } else {
          // Fallback for direct GitHub analysis without error context
          state.ticketContextForPR = {
  ...
  function renderFileContent() {
    ...
    // Handle pending error context (from Event Details Upload from GitHub)
    ...
    if (state.pendingGitHubErrorContext) {
      ...
      const analysisInputSection = document.getElementById(
  ...
  function formatAnalysis(text) {
    ...
    function severityBadge(text) {
      ...
      let cls = "severity-badge";

      if (upper.startsWith("CRITICAL")) cls += " severity-critical";
      else if (upper.startsWith("HIGH")) cls += " severity-high";
      else if (upper.startsWith("MEDIUM")) cls += " severity-medium";
      else if (upper.startsWith("LOW")) cls += " severity-low";
  ...
  function getEventDetailFlowButtonsHtml(state, logIndex, fileName) {
    const safeFile = escapeHtml(fileName || "N/A");
  ...
  function updateEventDetailFlowButtons() {
    ...
    modal.querySelectorAll(".event-detail-actions").forEach((container) => {
      const logIndex = parseInt(
        container.getAttribute("data-log-index") || "0",
      ...
      container.innerHTML = getEventDetailFlowButtonsHtml(
        state,
  ...
  function closeEventDetailsModal() {
    ...
    if (modal) {
      ...
      console.log("Event details modal closed");
    }

    // Clean up global context

    if (window.__eventDetailsContext) {
      delete window.__eventDetailsContext;
    }
  }
  ...
  // Analyze GitHub file content

  function analyzeGitHubFileContent(fileInfo, content) {
    // Create analysis modal

    const analysisModalHtml = `

      <div class="event-details-modal" id="githubAnalysisModal">
        ...
        <div class="event-details-content">
          ...
          <div class="event-details-header">
            ...
            <h3>AI Analysis - ${escapeHtml(fileInfo.name)}</h3>

            <button class="btn-icon" id="closeGitHubAnalysis">✕</button>
          ...
          <div class="event-details-body">
            ...
            <div class="analysis-input-section">
              ...
              <div class="analysis-form">
                ...
                <div class="text-input-container">
                  <textarea id="githubAnalysisPrompt" placeholder="Provide your input regarding the analysis here..."></textarea>
                  <button class="btn-primary" id="runGitHubAnalysis">Analyze</button>
            ...
            <div class="analysis-result" id="githubAnalysisResult"></div>
    ...
    document
      ...
      .getElementById("closeGitHubAnalysis")

      .addEventListener("click", () => {
        document.getElementById("githubAnalysisModal")?.remove();
      });

    document

      .querySelector("#githubAnalysisModal .event-details-backdrop")
    ...
    document
      ...
      .getElementById("runGitHubAnalysis")

      .addEventListener("click", async () => {
        const prompt = document.getElementById("githubAnalysisPrompt").value;

        const resultDiv = document.getElementById("githubAnalysisResult");

        resultDiv.innerHTML = '<div class="loading">Analyzing...</div>';

        try {
          const result = await api("POST", "/api/github/analyze", {
            content: content,
          ...
          if (result.success) {
            // Hide the input section after successful GitHub analysis
            const githubInputSection = document.querySelector(
              "#githubAnalysisModal .analysis-input-section",
            );
            if (githubInputSection) {
              githubInputSection.classList.add("hidden");
            }

            console.log("Raw GitHub analysis from LLM:", result.analysis);

            const formattedAnalysis = formatAnalysis(result.analysis);

            console.log(
              "Formatted GitHub analysis length:",

              formattedAnalysis.length,
            );

            resultDiv.innerHTML = `<div class="analysis-text">${formattedAnalysis}</div>`;

            // resultDiv.innerHTML = `<div class="analysis-text"><pre style="white-space: pre-wrap; font-family: inherit;">${result.analysis}</pre></div>`;
          } else {
            resultDiv.innerHTML = `<div class="error">Analysis failed: ${result.error}</div>`;
          }
        } catch (err) {
  ...
  // Attach GitHub file to error analysis

  function attachGitHubFileToError(fileInfo, content) {
    const ctx = window.__eventDetailsContext;

    if (!ctx) {
      alert("No error context available. Please open error details first.");

      return;
    }

    // Attach the GitHub file to the error context
    ...
    ctx.analysisFlowState = "attached";
      ...
      <div class="event-details-modal" id="aiAnalysisModal">
        ...
        <div class="event-details-content">
          ...
          <div class="event-details-body">
            ...
            <div class="analysis-actions" id="aiAnalysisActions">
              <!-- Generate Code Changes button will be added dynamically after analysis completes -->
            </div>
  ...
  function getFilteredErrorCount(logs) {
    if (!logs || logs.length === 0) return 0;

    return logs.filter((log) => isLogInTimeRange(log)).length;
  }
  ...
  // Anypoint Login Popup Functions
  ...
  window.loginAnypointFromPopup = async function () {
    const clientName = document
      ...
      .getElementById("popupClientName")

      .value.trim();

    const clientId = document

      .getElementById("popupClientId")

      .value.trim();

    const clientSecret = document

      .getElementById("popupClientSecret")
    ...
    try {
      ...
      if (result.success) {
        // Close modal and update session
        ...
        if (result.organization) {
          ...
          console.log(`[Connected App] Logged into organization: ${result.organization.name} (${result.organization.id})`);

          // You can add a notification or display this info on the UI

        }



        // Refresh session from backend to ensure sync across entire app

        await checkSession();

        // Load Anypoint applications immediately after successful login

        if (state.currentEnvId) {
          await loadApplications(state.currentEnvId);
        }

        // Switch to MuleSoft tab to show applications

        await switchTab("mulesoft");

        // Check if user is already authenticated with GitHub

        // If not, ask if they want to login to GitHub too
...
function closeEventDetailsModal() {
  const modal = document.getElementById("eventDetailsModal");
  if (modal) {
    modal.remove(); // Properly remove the modal from DOM
  }
}

// Close AI Analysis modal (alias for same function)
function closeAiAnalysisModal() {
  const modal = document.getElementById("aiAnalysisModal");
  if (modal) {
    modal.remove(); // Properly remove the modal from DOM
...
window.analyzeErrorWithRulesetUnified = async function(logIndex = 0, fileList = null, fileContents = null) {
  ...
  if (!resultDiv) {
    ...
    document.body.insertAdjacentHTML("beforeend", analysisModalHtml);
    document.getElementById("closeAiAnalysis").addEventListener("click", closeAiAnalysisModal);
    document.querySelector("#aiAnalysisModal .event-details-backdrop").addEventListener("click", closeAiAnalysisModal);
  }
...
// Expose state to global scope for serviceNow.js access
  window.state = state;
